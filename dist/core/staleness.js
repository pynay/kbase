 ...
 */
function lastCommitDate(repoRoot, relPath) {
    try {
        const out = execFileSync("git", ["log", "-1", "--format=%cI", "--", relPath], { cwd: repoRoot, encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] }).trim();
 ...
 */
export function checkStaleness(entry, entryPath, repoRoot) {
    if (!entry.timestamp)
        return null;
 ...
 */
export function checkAllStaleness(entries, repoRoot) {
    const results = [];

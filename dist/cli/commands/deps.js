 ...
 */
function printHuman(module, node, direction) {
    console.log(`\n  ${module}`);
 ...
 */
function printJson(module, node, direction) {
    const output = { module };
    if (direction === "up" || direction === "both") {
...
export function register(program) {
    program
        ...
        if (!node) {
            console.error(`Module "${module}" not found in the dependency index.\n` +
        ...
        if (options.up && !options.down)
            direction = "up";
        else if (options.down && !options.up)
            direction = "down";
        // 5. Render output
        if (options.json) {

...
const TOOLS = [
    {
        name: "read_knowledge",
        description: "Read existing knowledge about a module or file. Call this before " +
        ...
        inputSchema: {
            ...
            properties: {
                target: {
                    type: "string",
                    description: "Module name (e.g. 'auth/session') or file path (e.g. 'src/auth/session.ts').",
                },
                depth: {
                    type: "string",
                    enum: ["summary", "full"],
                    description: "summary = frontmatter-only per entry (cheap scan, omits decision body). " +
    ...
    },
    {
        ...
        description: "Create a new knowledge entry under .knowledge/<module>/<id>.md. " +
    ...
    {
        name: "query_deps",
        description: "Look up a module in the dependency graph. Returns upstream " +
...
async function handleReadKnowledge(args) {
    ...
    const depth = args.depth === "full" ? "full" : "summary";
    ...
    const payload = depth === "full" ? matched : matched.map(toFrontmatter);
    return jsonContent(payload);
}
async function handleWriteKnowledge(args) {
    // Validate the four required fields explicitly so we return a clean MCP
    // error instead of a cryptic core-library stack trace.
    const required = ["module", "summary", "decision", "files"];
...
async function handleQueryDeps(args) {
    ...
    const direction = rawDirection === "up" || rawDirection === "down" ? rawDirection : "both";
    const knowledgeDir = await resolveKnowledgeDir();
    const deps = await getDependencies(knowledgeDir);
    const node = deps[module];
    if (!node) {
        return errorContent(`Module "${module}" not found in the dependency index. ` +
...
export async function startServer() {
    const server = new Server({ name: "kbase", version: "0.1.0" }, { capabilities: { tools: {} } });

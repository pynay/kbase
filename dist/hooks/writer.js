...
import { readEntry, writeEntry, listEntryPaths, validateDecisionLength } from "../core/store.js";
...
export function buildWriterTools() {
    return [
        {
            name: "read_knowledge",
...
async function handleToolCall(name, input, knowledgeDir, config) {
    if (name === "read_knowledge") {
        ...
        if (moduleIndex[target]) {
            for (const id of moduleIndex[target])
                entryIds.add(id);
        }
        if (fileIndex[target]) {
            for (const id of fileIndex[target])
                entryIds.add(id);
        }
        if (entryIds.size === 0)
            return "[]";
...
export async function runWriter(payload, config) {
                ...
                for (const block of response.content) {
                    if (block.type === "tool_use") {
                        ...
                        while (retries <= MAX_RETRIES_PER_TOOL) {
                            try {
                                result = await handleToolCall(block.name, block.input, knowledgeDir, config);
                                if (block.name === "write_knowledge")
                                    entriesWritten++;
                                isError = false;
                                break;
                            }

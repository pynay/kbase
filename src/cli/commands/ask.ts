...
function scoreEntry(entry: KnowledgeEntry, keywords: string[]): number {
  ...
  for (const kw of keywords) {
    ...
    if (moduleLower.includes(kw)) score += 2;
  }
  return score;
}
 ...
 */
async function readReferencedSources(
  entries: KnowledgeEntry[],
  repoRoot: string
): Promise<Array<{ path: string; contents: string }>> {
  const fileSet = new Set<string>();
  for (const e of entries) for (const f of e.files ?? []) fileSet.add(f);
    ...
    .action(async (question: string, options: Record<string, unknown>) => {
      ...
      if (options.sources) {
        ...
        for (const e of ranked) {
          process.stdout.write(
            `    - ${e.module}/${e.id.slice(0, 8)}  ${e.summary}\n`
          );
        }
        process.stdout.write("\n");
      }

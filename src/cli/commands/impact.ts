...
import type { DependencyIndex, KnowledgeEntry } from "../../core/types.js";
...
function collectAffectedModules(
  ...
  for (const seed of seeds) {
    ...
    if (!node) continue;
    for (const m of node.depends_on) out.add(m);
    for (const m of node.depended_on_by) out.add(m);
  }
  return out;
}
...
function renderDependencyBlock(
  ...
  return directModules
    .map((m) => {
      const node = deps[m];
      if (!node) return `${m}: (not present in dependency graph)`;
      const up = node.depends_on.length > 0 ? node.depends_on.join(", ") : "(none)";
      const down =
        node.depended_on_by.length > 0 ? node.depended_on_by.join(", ") : "(none)";
      return `${m}\n  depends on:     ${up}\n  depended on by: ${down}`;
    })
    .join("\n\n");
}
    ...
    .action(async (file: string, options: Record<string, unknown>) => {
      ...
      for (const key of lookupKeys) {
        if (Object.prototype.hasOwnProperty.call(filesIndex, key)) {
          for (const id of filesIndex[key]) entryIds.add(id);
        }
      }
      ...
      if (options.json) {
        ...
        const depSlice: DependencyIndex = {};
        for (const m of affectedModules) {
          if (deps[m]) depSlice[m] = deps[m];
        }
        console.log(
          JSON.stringify(
            {

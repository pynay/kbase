 ...
 */
function printHumanStale(stale: StaleEntry, knowledgeDir: string): void {
  ...
  for (const f of stale.staleFiles) {
    console.log(
      `    • ${f.filePath}  (modified ${f.fileModified}, +${f.daysSinceEntry}d)`
    );
  }
}
...
export function register(program: Command): void {
  program
    ...
    .action(async (options: Record<string, unknown>) => {
      ...
      if (!repoRoot) {
        console.error(
          "Not inside a git repository — `kb stale` needs git history to\n" +
      ...
      const entries = await Promise.all(
        paths.map(async (p) => ({ path: p, entry: await readEntry(p) }))
      );
      ...
      for (const s of stale) printHumanStale(s, knowledgeDir);
      console.log();
    });
}
